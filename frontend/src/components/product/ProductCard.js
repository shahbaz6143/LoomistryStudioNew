...
export default function ProductCard({ product }) {
  ...
  const handleAddToCart = (e) => {
    ...
    if (!firstSize) return;
    addToCart({ productId: product._id, size: firstSize.label, color: product.colors?.[0] || '' });
  };
      ...
      <Link href={`/products/${product.slug}`} className={styles.imageWrapper}>
        ...
        {(!user || user.role === 'buyer') && (
          <div className={styles.quickActions}>
            <button className={styles.addToCart} onClick={handleAddToCart}>Add to Bag</button>
            <button className={styles.wishlist} onClick={handleWishlist} aria-label="Wishlist">
