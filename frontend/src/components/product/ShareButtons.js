...
export default function ShareButtons({ title, slug, image }) {
  const [copied, setCopied] = useState(false);
  const url = typeof window !== 'undefined' ? `${window.location.origin}/products/${slug}` : `https://loomistrystudio.com/products/${slug}`;

  const shareWhatsApp = () => {
    window.open(`https://wa.me/?text=${encodeURIComponent(`Check out this rug: ${title}\n${url}`)}`, '_blank');
  };
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.buttons}>
        <button onClick={shareWhatsApp} className={styles.btn} aria-label="Share on WhatsApp" title="WhatsApp">
        ...
        <button onClick={sharePinterest} className={styles.btn} aria-label="Share on Pinterest" title="Pinterest">
        ...
        <button onClick={copyLink} className={`${styles.btn} ${copied ? styles.copied : ''}`} aria-label="Copy link" title={copied ? 'Copied!' : 'Copy link'}>
          {copied ? (
